...
function App() {
    ...
    const [openEditUI, setOpendEditUI] = useState(false)
        ...
        <div className="flex flex-col w-full p-10">
            ...
            <div className="mt-5 flex flex-col space-y-5 sm:space-y-0 sm:grid sm:grid-cols-2 sm:gap-10 lg:grid-cols-3">
                {todos?.map((todo, index) => (
                    ...
                    >
                    <p onClick={() => {setEditStatus(todo.status)
            ...
            <div className={`w-72 h-fit bg-white text-slate-900 absolute left-1/2 rounded xl px-3 py-2 -translate-x-1/2 -translate-y-1/2 ${openEditUI ? "" : 'hidden'}`}>
            <div className="flex items justify-between">
                <h1 className="text-xl mb-2">Edit Todos</h1>
                <i onClick={() => setOpendEditUI(false)}>
                ...
                <div>
                    <input type="text"
                           ...
                           placeholder="Change here name" value={editName}
